...
const emptyForm = { officeId: "", userId: "", tableNumber: "", reservationDate: toDateInput(), reason: "", replacementTableNumber: "" };
...
function AdminReservations() {
  ...
  function openCreate() { setForm({ ...emptyForm, officeId: selectedOfficeId }); setModal({ mode: "create" }); }
  function openEdit(item) {
    setForm({ officeId: item.table?.office?.id ?? selectedOfficeId, userId: item.userId ?? item.user?.id ?? "", tableNumber: String(item.table?.number ?? ""), reservationDate: item.reservationDate?.slice(0, 10) ?? toDateInput(), reason: "", replacementTableNumber: "" });
  ...
  return (
    <>
      ...
      <div className="admin-toolbar">
        <select className="admin-filter" value={selectedOfficeId} onChange={(event) => setSelectedOfficeId(event.target.value)}>
          {offices.map((office) => <option key={office.id} value={office.id}>{office.city}</option>)}
        </select>
      ...
      {modal && <Modal title={modal.mode === "edit" ? "Edit reservation" : "Create reservation"} description="The impact will be previewed before changes are applied." onClose={() => setModal(null)}>
        <form className="admin-form" onSubmit={submit}><div className="admin-form-grid">
          <div className="admin-field full"><label>Office</label><select required name="officeId" value={form.officeId} onChange={updateField}>{offices.map((office) => <option key={office.id} value={office.id}>{office.city} · {office.name}</option>)}</select></div>
          ...
          <div className="admin-field"><label>Desk number</label><input required min="1" max={formTableLimit} type="number" name="tableNumber" value={form.tableNumber} onChange={updateField} /></div>
          ...
          {modal.mode === "create" && <div className="admin-field"><label>Replacement desk (optional)</label><input min="1" max={formTableLimit} type="number" name="replacementTableNumber" value={form.replacementTableNumber} onChange={updateField} /><small>Used to relocate the current occupant of the target desk.</small></div>}

...
const emptyForm = { officeId: "", userId: "", tableNumber: "", startsOn: toDateInput(), endsOn: "", reason: "" };
...
function AdminAssignments() {
  ...
  const [offices, setOffices] = useState([]); const [selectedOfficeId, setSelectedOfficeId] = useState("");
  const formTableLimit = offices.find((office) => office.id === form.officeId)?.city.toLowerCase() === "izmir" ? 16 : 32;

  useEffect(() => {
    adminApi.getOffices().then((officeItems) => {
      setOffices(officeItems);
      const istanbul = officeItems.find((office) => office.city.toLowerCase() === "istanbul");
      setSelectedOfficeId((istanbul ?? officeItems[0])?.id ?? "");
  ...
  const visible = useMemo(() => items.filter((item) => {
    ...
    const matchesOffice = !selectedOfficeId || item.table?.office?.id === selectedOfficeId;
    return matches && matchesOffice && (showRevoked || !item.revokedAt);
  }), [items, query, showRevoked, selectedOfficeId]);

  function field(event) { setForm((current) => ({ ...current, [event.target.name]: event.target.value })); }
  async function submit(event) {
    event.preventDefault(); setBusy(true);
    const payload = { officeId: form.officeId, userId: form.userId, tableNumber: Number(form.tableNumber), startsOn: form.startsOn, endsOn: form.endsOn || null, ...(form.reason.trim() ? { reason: form.reason.trim() } : {}) };
    try {
      ...
      toast.success("Desk assignment created."); setOpen(false); setForm({ ...emptyForm, officeId: selectedOfficeId }); await load();
  ...
  return <>
    <PageHeading eyebrow="Long-term seating" title="Permanent assignments" description="Reserve a desk for a user across an inclusive date range." action={<button type="button" className="admin-button primary" onClick={() => { setForm({ ...emptyForm, officeId: selectedOfficeId }); setOpen(true); }}><Plus size={16} /> New assignment</button>} />
    <div className="admin-toolbar"><select className="admin-filter" value={selectedOfficeId} onChange={(event) => setSelectedOfficeId(event.target.value)}>{offices.map((office) => <option key={office.id} value={office.id}>{office.city}</option>)}</select><div className="admin-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search user or desk" /></div><label className="admin-equipment-option"><input type="checkbox" checked={showRevoked} onChange={(event) => setShowRevoked(event.target.checked)} /> Show revoked</label></div>
    <section className="admin-card">{loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={load} /> : visible.length === 0 ? <EmptyState title="No assignments found" description="Create the first long-term desk assignment." /> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>User</th><th>Desk</th><th>Starts</th><th>Ends</th><th>Status</th><th style={{ textAlign: "right" }}>Actions</th></tr></thead><tbody>
      {visible.map((item) => <tr key={item.id}><td><div className="admin-person"><span className="admin-person-avatar">{item.user?.fullName?.[0] ?? "U"}</span><div><strong>{item.user?.fullName}</strong><span>{item.user?.email}</span></div></div></td><td><strong>{item.table?.code ?? item.table?.number}</strong><span>{item.table?.office?.city}</span></td><td>{formatDate(item.startsOn)}</td><td>{formatDate(item.endsOn)}</td><td><StatusPill tone={item.revokedAt ? "danger" : "info"}>{item.revokedAt ? "Revoked" : "Assigned"}</StatusPill></td><td><div className="admin-row-actions">{!item.revokedAt && <><button type="button" className="admin-icon-action" onClick={() => updateEndDate(item)} title="Update end date"><Edit3 size={15} /></button><button type="button" className="admin-icon-action danger" onClick={() => revoke(item)} title="End assignment"><XCircle size={15} /></button></>}</div></td></tr>)}
    ...
    {open && <Modal title="Create permanent assignment" description="Assignments take priority over normal user reservations." onClose={() => setOpen(false)}><form className="admin-form" onSubmit={submit}><div className="admin-form-grid">
      <div className="admin-field full"><label>Office</label><select required name="officeId" value={form.officeId} onChange={field}>{offices.map((office) => <option key={office.id} value={office.id}>{office.city} · {office.name}</option>)}</select></div>
      ...
      <div className="admin-field"><label>Desk number</label><input required min="1" max={formTableLimit} type="number" name="tableNumber" value={form.tableNumber} onChange={field} /></div>
